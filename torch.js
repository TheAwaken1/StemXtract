  ...
  run: [
    ...
    // windows amd
    {
      ...
      "params": {
        "venv": "{{args && args.venv ? args.venv : null}}",
        "path": "{{args && args.path ? args.path : '.'}}",
        "message": "uv pip install torch-directml==2.0.0 torchaudio==2.0.1 torchvision==0.15.1 numpy==1.26.4"
      },
      "next": null
    },
    // windows cpu
    ...
    // linux nvidia
    {
      ...
      "params": {
        ...
        "path": "{{args && args.path ? args.path : '.'}}",
        "message": [
          "uv pip install torch torchvision torchaudio numpy==1.26.4 {{args && args.xformers ? 'xformers' : ''}} --index-url https://download.pytorch.org/whl/cu124",
          "{{args && args.sageattention ? 'uv pip install git+https://github.com/thu-ml/SageAttention.git' : ''}}"
        ]
      },
      "next": null
    ...
    // linux rocm (amd)
    {
      ...
      "params": {
        "venv": "{{args && args.venv ? args.venv : null}}",
        "path": "{{args && args.path ? args.path : '.'}}",
        "message": "uv pip install torch torchvision torchaudio numpy==1.26.4 --index-url https://download.pytorch.org/whl/rocm6.2.4"
      },
      "next": null
    },
    // linux cpu
    {
      ...
      "params": {
        "venv": "{{args && args.venv ? args.venv : null}}",
        "path": "{{args && args.path ? args.path : '.'}}",
        "message": "uv pip install torch torchvision torchaudio numpy==1.26.4 --index-url https://download.pytorch.org/whl/cpu"
      },
      "next": null
    }
  ]
